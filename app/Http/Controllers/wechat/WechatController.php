...
class WechatController extends Controller
{
    ...
    public function get_user_list()
    {
       ...
       $openid =  $user_info['data']['openid'];
            ...
            for ($i=0;$i<count($openid);$i++){
                ...
                //获取openid
                $user_openid = DB::connection('mysql_shop3')->table('wechat_openid')->where('openid',$user_info['openid'])->first();
                if(empty($user_openid)){
                    $openid = $openid[$i];
                    DB::connection('mysql_shop3')->table('wechat_openid')->insert([
                        'openid' => $openid,
                        'add_time' => time(),
    ...
    public function user_list()
    {
       ...
       $user = DB::connection('mysql_shop3')->table('wechat_openid')->get()->toArray();
    ...
    public static function login()
    {
        ...
        $redirect_uri='http://ww.tp.com/wechat/code';
...
//发送模板消息
    public function send_template()
    {
       $template_id = $this->request->all()['id'];
        $openid = DB::connection('mysql_shop3')->table('wechat_openid')->select('openid')->limit(10)->get()->toArray();
...
//为用户添加标签
    public function add_user_tag()
    {
       ...
       $id = $this->request->all()['id_list'];
        $openid = DB::connection('mysql_shop3')->table('wechat_openid')->whereIn('id', $id)->select('openid')->get()->toArray();
        $openid_list = [];
        foreach ($openid as $v) {
